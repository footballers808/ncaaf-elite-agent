    ...
    for g in games:
        ...
        for side in ("home","away"):
            ...
            rows.append({
                ...
                "spread_close": g.get("spread"),
                "total_close": g.get("over_under"),
...
def _pace_from_stats(year: int, season_type: str) -> pd.DataFrame:
    ...
    for g in stats:
        ...
        for team in g.get("teams", []):
            ...
            plays = float(stat_map.get("plays", 0) or 0)
...
def build_features(years: List[int], season_type: str, cfg: Dict[str, Any]):
    frames = []
    # Try to load optional news features once
    try:
        news_df = pd.read_parquet("artifacts/features/news.parquet")
    except Exception:
        news_df = None
    ...
    for y in years:
        ...
        if news_df is not None:
            df = df.merge(news_df, on="team", how="left")
        frames.append(df)
