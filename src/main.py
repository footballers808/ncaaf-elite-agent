...
def _resolve(module_name: str, candidates: List[str]) -> Optional[Callable]:
    """
    Import a sibling module (e.g., .model) and return the first attribute that exists
...
def _resolve_model_api():
    predict_fn = _resolve(".model", ["predict_games", "predict", "run_predict", "make_predictions"])
    train_fn   = _resolve(".model", ["train_model", "train", "learn", "fit_model", "fit"])
    save_fn    = _resolve(".model", ["save_model", "save", "persist_model"])
    load_fn    = _resolve(".model", ["load_model", "load", "load_or_init_model", "init_model"])
    return predict_fn, train_fn, save_fn, load_fn

def _resolve_labeler_api():
    label_fn = _resolve(".labeler", ["label_latest_results", "label_results", "build_labels", "label"])
    return label_fn

# ------------------------- Stars / edge helpers (same as before) -------------------------
